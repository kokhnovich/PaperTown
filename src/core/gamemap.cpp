...
GameMap::GameMap(QObject *parent, int size_n, int size_m) :
    QObject(parent), map_(size_n, QVector<GameObject*>(size_m, nullptr))
{
}
...
void GameMap::moved(const Coordinate &oldPosition, const Coordinate &)
{
    ...
    for (const Coordinate &coord : qAsConst(res)) {
